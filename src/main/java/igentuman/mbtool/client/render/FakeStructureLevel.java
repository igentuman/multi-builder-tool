...
/**
 * Minimal {@link BlockAndTintGetter} backed by a plain {@code BlockPos -> BlockState} map.
 *
 * <p>Positions outside the structure resolve to AIR, so the block renderer culls internal
 * faces between structure blocks but keeps every boundary face. Passing this view to
 * {@code ModelBlockRenderer#tesselateBlock} lets connected-texture (CTM) and model-data aware
 * models inspect their neighbours, which the old direct {@code model.getQuads()} path could not.</p>
 *
 * <p>Light queries return full-bright so the ghost preview is always clearly visible.</p>
 */
@OnlyIn(Dist.CLIENT)
