...
/**
 * Bakes a {@link FakeStructureLevel} into per-render-layer {@link VertexBuffer}s once, then draws
 * them straight from GPU memory every frame. Replaces the old per-frame CPU tessellation.
 *
 * <p>Because geometry is produced through {@link ModelBlockRenderer#tesselateBlock} with a level
 * view, connected textures (CTM), model data and internal face culling all work correctly.</p>
 */
@OnlyIn(Dist.CLIENT)
public class StructureMesh {
    ...
    /** Rebuilds all VBOs from the given fake level. Cheap enough to call only on change. */
    public void rebuild(FakeStructureLevel level) {
        ...
        for (RenderType rt : CHUNK_LAYERS) {
            ByteBufferBuilder bbb = new ByteBufferBuilder(131072); // 128KB per layer
        ...
        // Close underlying pool builders
    ...
    /**
     * Draws every baked layer as a translucent ghost with the terrain showing through
     * (depth test off). Used for the in-world placement preview.
     *
     * @param alpha whole-mesh alpha, multiplied into the block shader colour
     */
    ...
    /**
     * Draws every baked layer.
     *
     * @param alpha     whole-mesh alpha, multiplied into the block shader colour
     * @param depthTest {@code true} for solid GUI previews (front blocks occlude back);
     *                  {@code false} for the see-through in-world ghost
     */
